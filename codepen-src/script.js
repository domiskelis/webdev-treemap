...
class Grapher extends React.Component {
  ...
  render() {
    ...
    Object.keys(buttonResponder).forEach(id => {
      grapherButtons.push(
          <button class="button" type="button" id={id}
                             onClick={this.changeGraph}
                             onMouseEnter={(e) => {this.handleMouseOver(e)}}
                             onMouseLeave={this.handleMouseOut}
                             onMouseMove={this.handleMouseMove}>
              <i class={buttonResponder[id].icon}/></button>
      );
    });
    ...
    if (this.state.hasOwnProperty("url") > 0) {
      description.push(
        <span>{this.state.description}. Data retrieved from <a href={this.state.url}>this JSON</a>.</span>);
      } else { description.push(this.state.description); };
    //render page
    return (
      <div id="wrapper">
        <div id="head-block" class="non-graph">
          <h1 id="title">{this.state.title}</h1>
          <div id="description" class="description-container">{description}</div>
          <div id="toggles" class="toggles-container">
            <div id="button-label" class="button-label"/>
            <div id="data-button-label">Display a dataset:</div>
            <div id="or-text">OR</div>
            <div id="palette-button-label">Shuffle colors:</div>
            {grapherButtons}
          </div>
        </div>
        <div id="graph-box" class="graph-box">
